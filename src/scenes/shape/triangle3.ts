...
export class Triangle3Composition extends Composition {
    animated = false;

    iterationLimit = Math.round(Math.random() * 3 + 5);
    branchingFactor = 3;
    ...
    drawNext(p: p5, iterations: number, colors: Colors, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): void {
        ...
        const newMiddleX = (x1 + x2 + x3) / 3;
        const newMiddleY = (y1 + y2 + y3) / 3;

        // bottom triangle
        const topX1 = newMiddleX;
        const topY1 = newMiddleY;
        const topX2 = x2;
        const topY2 = y2;
        const topX3 = x3;
        const topY3 = y3;
        this.drawNext(p, iterations - 1, colors.copy(), topX1, topY1, topX2, topY2, topX3, topY3);

        // left triangle
        const leftX1 = newMiddleX;
        const leftY1 = newMiddleY;
        const leftX2 = x3;
        const leftY2 = y3;
        const leftX3 = x1;
        const leftY3 = y1;
        ...
        // right triangle
        const rightX1 = newMiddleX;
        const rightY1 = newMiddleY;
        const rightX2 = x1;
        const rightY2 = y1;
        const rightX3 = x2;
        const rightY3 = y2;
